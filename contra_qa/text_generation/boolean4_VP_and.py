...
import pandas as pd
try:
    from word_lists import name_list, city_list, professions, color_list, city_list  # noqa
except ImportError:
    from contra_qa.text_generation.word_lists import name_list, city_list, professions, color_list, city_list  # noqa

try:
    from word_lists import verbs_third_person_past, verbs_third_person_past_CONTEXT  # noqa
except ImportError:
    from contra_qa.text_generation.word_lists import verbs_third_person_past, verbs_third_person_past_CONTEXT  # noqa


try:
    from word_lists import verbs_third_person_past_infinitiv_neg, positive_personality_list  # noqa
except ImportError:
    from contra_qa.text_generation.word_lists import verbs_third_person_past_infinitiv_neg, positive_personality_list  # noqa
import os
...
def boolean4():
    ...
    for i in range(int(upper_bound)):
        ...
        complement1 = replace_words(
            complement1, person1, color1, city1, profession1, pred1)
        context = verbs_third_person_past_CONTEXT[verb2]
        complement2 = get_new_item([], context)
        complement2 = replace_words(
            complement2, person2, color2, city2, profession2, pred2)
        if i % 2 == 0:
    ...
    for i in range(int(upper_bound)):
        ...
        other_person = get_new_item(
            [person, person1, person2, person3], name_list)
        color1 = get_new_item([], color_list)
        color2 = get_new_item([color1], color_list)
        ...
        complement3 = replace_words(
            complement3, person3, color3, city3, profession3, pred3)
        if i % 3 == 0:
            new_person = person
